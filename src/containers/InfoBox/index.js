...
import { compose, renderNothing, branch, withHandlers, withProps } from 'recompose';
...
const SubLabel = ({ children }) => <div className={styles.subLabel}>{ children }</div>;


const InfoBox = ({ selection, materials, handleRemove, handlePropertyChange }) =>
  <Container>
    <Label>Name</Label>
      ...
      <div key={material.name}>
        ...
        { material.options.map(option =>
          <Thumbnail
            ...
            onClick={() => handlePropertyChange({ [material.name]: option.key })}
            key={option.key}
            src={option.imageUrl}
          />,
        )}
...
export default compose(
  ...
  withProps(({ selection }) => ({
      viewarApi,
      materials: Object.values(selection.properties).filter(p => p.type === 'material' && p.options.length > 1)
  })),
  withLoading(),
  withHandlers({
